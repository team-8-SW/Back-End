...
export const getfeedposts = async (user_id: string): Promise<posts[]> => {
	try {
		...
		const userIds = await knexInstance.select('user_id').from(function (
				...
				.unionAll(function (this: Knex.QueryBuilder) {
					this.select(
						knexInstance.raw(
							...
							[user_id],
						), // Get user_id from connections
					)
						.from('connections')
						.where(function (this: Knex.QueryBuilder) {
							this.where('requester_id', user_id).orWhere('receiver_id', user_id);
						})
...
export const like = async (like: {
	...
	try {
		...
		if (post_id) {
			...
			const likeExists = await knexInstance('likes').where({ user_id, post_id }).first();
			if (likeExists) {
			...
			const commentExists = await knexInstance('comments').where({ id: comment_id }).first();
			if (!commentExists) {
		...
		if (post_id) {
			await knexInstance('posts').where({ id: post_id }).increment('like_count', 1);
		} else if (comment_id) {
			await knexInstance('comments').where({ id: comment_id }).increment('like_count', 1);
		}
		...
		if (postOwner) {
			...
			notifyUser(
				...
				user_id,
			); //kda fil notification bab3at el postowner.user_id da el user name beta3 ely haygilo el notification, w bab3at elnotification w bab3at el id beta3 el user ely 3amal el like
		}
		return createdLike;
	} catch (error) {
...
export const tagUser = async (tag: {
	post_id?: string;
	comment_id?: string;
	user_id: string;
	...
	try {
		...
		if (post_id) {
			// Ensure the post exists
			const postExists = await knexInstance('posts').where({ id: post_id, user_id }).first();
			if (postExists) {
...
export const commentpost = async (comment: {
	...
	try {
		...
		if (!parent_comment_id) {
			await knexInstance('posts').where({ id: post_id }).increment('comment_count', 1);
...
//viewpostengagement
export const viewpostengagement = async (engagement: {
	post_id: string;
	user_id: string;
}): Promise<any> => {
	try {
...
export const share = async (share: { post_id: string; user_id: string }): Promise<any> => {
	try {
		...
		await knexInstance('posts').where({ id: post_id }).increment('repost_count', 1);
		return shared;
	} catch (error) {
...
export const deletepost = async (share: { post_id: string; user_id: string }): Promise<any> => {
	try {
		...
		const mypost = await knexInstance('posts').where({ id: post_id, user_id: user_id }).first();
		if (!mypost) {
...
export const searchpost = async (share: { keyword: string }): Promise<any> => {
	try {
		...
		const results = await knexInstance('posts').where('content', 'like', `%${keyword}%`); // Search for the keyword in the 'content' column
		return results;
	} catch (error) {
