...
export const likePost = async (req: Request, res: Response) => {
	try {
		...
		if (post_id && comment_id) {
			return res
				.status(400)
				.json({ message: 'you cant like both comment and post at same time' });
		}
		// Call the service to create the post
		...
		if (file) {
			...
			if (!allowedMimeTypes.includes(file.mimetype)) {
				return res.status(400).json({
					error: 'Invalid file type. Only JPEG, PNG, MP4, and PDF are allowed.',
				});
			}
			...
			try {
				...
				const updatedPost = await postService.addMediaToPost(
					post_id,
					user_id,
					result.secure_url,
					file.mimetype,
